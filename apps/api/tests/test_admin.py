...
async def test_admin_users_disable_and_audit(
    ...
    logs = await client.get("/api/admin/audit-logs", params={"action": "admin.user.update"})
...
async def test_admin_oauth_client_crud(
    ...
    listed = await client.get("/api/admin/oauth/clients")
    assert listed.status_code == 200
    assert any(c["client_id"] == "admin-demo" for c in listed.json()["items"])

    updated = await client.patch(
        "/api/admin/oauth/clients/admin-demo",
        json={"name": "Admin Demo Renamed"},
    )
    assert updated.status_code == 200
    assert updated.json()["name"] == "Admin Demo Renamed"

    deleted = await client.delete("/api/admin/oauth/clients/admin-demo")
    assert deleted.status_code == 200

    listed2 = await client.get("/api/admin/oauth/clients")
...
async def test_admin_cannot_disable_self(
    ...
    uid = me.json()["id"]
    res = await client.patch(f"/api/admin/users/{uid}", json={"is_active": False})
